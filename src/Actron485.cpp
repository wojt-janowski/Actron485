...
namespace Actron485 {
    ...
    void Controller::applySlave3ReadResponse(uint16_t startAddress, uint16_t regCount, const uint8_t *data) {
        if (_slaveResponderEnabled && _slaveResponderId == 3) return;
    ...
    void Controller::applySlave11StateBroadcast(uint16_t startAddress, uint16_t regCount, const uint8_t *data, uint8_t byteCount) {
        ...
        if (_slaveResponderEnabled && _slaveResponderId == 3) {
            // This is our own output echoed by the AMIB, not new user intent.
    ...
    void Controller::loop() {
        if (_slaveResponderEnabled && uint32_t(platformMillis()-_thermalLastTick) >= 1000) {
            _thermalLastTick = uint32_t(platformMillis());
            tickThermostat();
            renderSlave3State();
        }
        if (_serial == nullptr) {
    ...
    void Controller::setSlaveResponderMode(uint8_t slaveId, bool enabled) {
        if (_slaveResponderEnabled != enabled) _thermostat.reset(uint32_t(platformMillis()));
    ...
    uint16_t Controller::encodeSlave3ZoneRegister(double setpoint, double currentTemperature) {
        if (!std::isfinite(setpoint)) return 0x2C00; // 22 C / no offset, with demand inhibited
    ...
    void Controller::renderSlave3State() {
        ...
        // Requested mode remains separate from the thermostat's wire branch.
        OperatingMode mode = stateMessage2.initialised ? stateMessage2.operatingMode : OperatingMode::Off;
        const auto &thermal = _thermostat.output();
        OperatingMode wireMode = mode;
        if (mode == OperatingMode::Auto) {
            wireMode = thermal.branch == OperatingMode::Off ? OperatingMode::Cool : thermal.branch;
        }
        uint8_t reg2Hi = wireMode == OperatingMode::Heat ? 0x01 :
            wireMode == OperatingMode::Cool ? 0x02 : wireMode == OperatingMode::FanOnly ? 0x08 : 0x00;
        uint8_t reg2Lo = (wireMode == OperatingMode::Heat || wireMode == OperatingMode::Cool)
            && thermal.branch == wireMode ? thermal.demand : 0;
        ...
        uint8_t reg3Lo;
        FanMode outputFan = stateMessage2.fanMode;
        if (mode != OperatingMode::FanOnly &&
            (outputFan == FanMode::Esp || outputFan == FanMode::EspContinuous)) {
            outputFan = thermal.demand >= 60 ? FanMode::High :
                thermal.demand >= 35 ? FanMode::Medium :
                thermal.demand > 0 ? FanMode::Low : FanMode::Esp;
        }
        switch (outputFan) {
            case FanMode::Low:
        ...
        for (int z = 0; z < 8; z++) {
            _slaveRegisters[5 + z] = encodeSlave3ZoneRegister(
                effectiveZoneTarget(z), isZoneSensorFresh(z+1) ? _sensorTemperature[z] : NAN);
        }
        if (mode != OperatingMode::Off) {
            stateMessage2.compressorMode = thermal.demand == 0 ? CompressorMode::Idle :
                thermal.branch == OperatingMode::Heat ? CompressorMode::Heating : CompressorMode::Cooling;
            stateMessage2.fanActive = (reg3Hi & 0x3A) != 0;
            stateMessage2.runningFanMode = stateMessage2.fanActive ? outputFan : FanMode::Off;
        } else {
            stateMessage2.compressorMode = CompressorMode::Idle;
            stateMessage2.fanActive = false;
            stateMessage2.runningFanMode = FanMode::Off;
        }
    }
    ...
    void Controller::setOperatingMode(OperatingMode mode) {
        ...
        if (_slaveResponderEnabled && _slaveResponderId == 3) {
            ...
            if (mode == OperatingMode::Auto) setMasterSetpoint((_autoLow+_autoHigh)/2);
    ...
    void Controller::setMasterSetpoint(double temperature) {
        if (!std::isfinite(temperature) || temperature < 16 || temperature > 30) return;
        if (!allowControlCommand()) {
            return;
        }

        if (_slaveResponderEnabled && stateMessage2.operatingMode == OperatingMode::Auto) {
            const double half=(_autoHigh-_autoLow)/2;
            temperature=std::clamp(temperature,16.0+half,30.0-half);
        }
        ...
        if (_slaveResponderEnabled && _slaveResponderId == 3) {
            for (int z = 0; z < 8; z++) {
                if (zoneControlled[z] || stateMessage2.zoneOn[z]) {
                    zoneSetpoint[z] = temperature;
                }
            }
            stateMessage2.initialised = true;
            stateMessage2.setpoint = temperature;
            if (stateMessage2.operatingMode == OperatingMode::Auto) {
                const double half=(_autoHigh-_autoLow)/2;
                _autoLow=std::max(16.0,temperature-half);
                _autoHigh=std::min(30.0,temperature+half);
            }
            tickThermostat();
            renderSlave3State();
        }
    ...
    double Controller::getMasterCurrentTemperature() {
        if (_slaveResponderEnabled && _slaveResponderId == 3) {
            ...
            for (int z=0;z<8;++z) if (stateMessage2.zoneOn[z] && isZoneSensorFresh(z+1)) {
                sum+=_sensorTemperature[z]; ++count;
            }
    ...
    void Controller::setZoneSetpointTemperatureCustom(uint8_t zone, double temperature, bool adjustMaster) {
        if (zone < 1 || zone > 8 || !std::isfinite(temperature) || temperature < 16 || temperature > 30) return;
        if (!allowControlCommand()) {
            return;
        }
        ...
        if (_slaveResponderEnabled && _slaveResponderId == 3) {
            ...
            // Auto per-zone targets are independent centers; preserve the
            // master comfort-band midpoint when adjusting one zone.
            if (zoneControlled[zindex(zone)] && stateMessage2.operatingMode != OperatingMode::Auto) {
                stateMessage2.initialised = true;
                stateMessage2.setpoint = temperature;
            }
            tickThermostat();
    ...
    void Controller::setZoneSetpointTemperature(uint8_t zone, double temperature, bool adjustMaster) {
        if (zone < 1 || zone > 8 || !std::isfinite(temperature) || temperature < 16 || temperature > 30) return;
        if (_slaveResponderEnabled && _slaveResponderId == 3) {
            setZoneSetpointTemperatureCustom(zone,temperature,adjustMaster);
            return;
        }
        if (!allowControlCommand()) {
            return;
    ...
    void Controller::setZoneCurrentTemperature(uint8_t zone, double temperature) {
        ...
        _sensorTemperature[zindex(zone)] = temperature;
        _sensorUpdated[zindex(zone)] = uint32_t(platformMillis());
        _sensorSeen[zindex(zone)] = std::isfinite(temperature) && temperature >= 0 && temperature <= 60;
        if (_slaveResponderEnabled && _slaveResponderId == 3) {
            tickThermostat();
            renderSlave3State();
        }
    ...
    bool Controller::isZoneSensorFresh(uint8_t zone) const {
        return zone >= 1 && zone <= 8 && _sensorSeen[zone-1] &&
            uint32_t(uint32_t(platformMillis())-_sensorUpdated[zone-1]) <= SENSOR_FRESH_MS;
    }

    bool Controller::setAutoComfortRange(double low, double high) {
        if (!std::isfinite(low) || !std::isfinite(high) || low < 16 || high > 30 || high-low < 1) return false;
        _autoLow=low;
        _autoHigh=high;
        if (_slaveResponderEnabled && stateMessage2.operatingMode == OperatingMode::Auto)
            setMasterSetpoint((low+high)/2);
        return true;
    }

    double Controller::effectiveZoneTarget(int z) const {
        if (stateMessage2.operatingMode != OperatingMode::Auto) return zoneSetpoint[z];
        const double mid=(_autoLow+_autoHigh)/2;
        const double target = _thermostat.output().branch == OperatingMode::Heat ? _autoLow : _autoHigh;
        return std::clamp(zoneSetpoint[z]+target-mid,16.0,30.0);
    }

    void Controller::tickThermostat() {
        if (!_slaveResponderEnabled || _slaveResponderId != 3) return;
        std::array<QueThermostat::Zone,8> zones{};
        const bool automatic = stateMessage2.operatingMode == OperatingMode::Auto;
        const double halfBand = (_autoHigh-_autoLow)/2;
        for (int z=0;z<8;++z) {
            const bool validTarget=std::isfinite(zoneSetpoint[z]) && zoneSetpoint[z]>=16 && zoneSetpoint[z]<=30;
            zones[z] = {stateMessage2.zoneOn[z],isZoneSensorFresh(z+1),_sensorTemperature[z],
                validTarget ? std::max(16.0,zoneSetpoint[z]-(automatic ? halfBand : 0)) : NAN,
                validTarget ? std::min(30.0,zoneSetpoint[z]+(automatic ? halfBand : 0)) : NAN};
        }
        _thermostat.step(uint32_t(platformMillis()),stateMessage2.operatingMode,zones);
    }
    ...
    double Controller::getZoneCurrentTemperature(uint8_t zone) {
        ...
        if (zone >= 1 && zone <= 8) {
            if (_slaveResponderEnabled && _slaveResponderId == 3)
                return isZoneSensorFresh(zone) ? _sensorTemperature[zindex(zone)] : NAN;
